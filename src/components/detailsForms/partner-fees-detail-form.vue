...
export interface UpdatePartnerFeesForm {
  id:number
  partner_id : number
...
const props = defineProps<{fees: UpdatePartnerFeesForm , activateEdition:boolean}>()

const formSize = ref('default')
const ruleFormRef = ref<FormInstance>()
const updatePartnerFeesForm = reactive<UpdatePartnerFeesForm>({
  id: props.fees.id,
  partner_id : props.fees.partner_id,
...
const rules = reactive<FormRules<UpdatePartnerFeesForm>>({
  base_country_id: [
...
const doUpdatePartnerFeesRequest = async (feesData : UpdatePartnerFeesForm):Promise<(formInstnace: (FormInstance | undefined), loader: any) => void>=>{
  const requestData =  await supabase.from('partner_fees')
      .update([PartnerFeesRequestMappper.updateFormToRequest(feesData)])
      .eq("id" , feesData.id)
      .select();
  return (formInstnace : FormInstance | undefined , loader: any)=>{
    if(!requestData.error){
      ...
      ElNotification({
        title: "Mise à jour de frais partnaire",
        message: "le frais partenaire à  été mise à jour avec success",
        type: 'success',
      })
      Logger.info("Partner Fees Updated successfully "+requestData.data)
    }else{
      ElNotification({
        title: "Mise à jour de frais partenaire",
        message: GlobalErrorHandler.process_error(requestData.status),
        type: 'error',
      })
      Logger.error("Partner Fees Update Error "+requestData.error)
    }
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async(valid, fields) => {
    if (valid) {
      const loader = Loader()
      const _handleError = await doUpdatePartnerFeesRequest(updatePartnerFeesForm)
    ...
    <el-collapse-item name="1" >
      ...
      >
        <el-form-item label="pays de base" prop="base_country_id">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun pays trouvé" placeholder="pays de base" v-model="updatePartnerFeesForm.base_country_id">
            <el-option
        ...
        <el-form-item label="pays de destination" prop="target_country_id">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun pays trouvé" placeholder="pays de destination" v-model="updatePartnerFeesForm.target_country_id">
            <el-option
                v-for="item in countryData"
                :key="item.id"
                :label="item.country_name"
        ...
        <el-form-item :label="'borne inferieure ('+updatePartnerFeesForm.base_country?.currency+')'" prop="currency">
          <el-input v-model="updatePartnerFeesForm.lower_bound" type="number" />
        </el-form-item>
        <el-form-item :label="'borne superieure ('+updatePartnerFeesForm.base_country?.currency+')'" prop="currency">
          <el-input v-model="updatePartnerFeesForm.upper_bound" type="number" />
        </el-form-item>
        <el-form-item label="type de frais" prop="fees_type">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun type de frais trouvé" placeholder="type de frais" v-model="updatePartnerFeesForm.fees_type">
            <el-option
        ...
        <el-form-item :label="'valeur du frais ('+(updatePartnerFeesForm.fees_type === 1 ? updatePartnerFeesForm.base_country.currency : '%') +')'" prop="currency">
          <el-input v-model="updatePartnerFeesForm.fees" type="number" />
        </el-form-item>
        <el-form-item>
