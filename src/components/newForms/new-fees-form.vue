...
const newFeesForm = reactive<NewFeesForm>({
  base_country_id: '',
  target_country_id: '',
  fees : '' ,
  fees_type : '' ,
...
const rules = reactive<FormRules<NewFeesForm>>({
  ...
  target_country_id: [
    ...
    {validator : FormValidator.shouldNotBeEqual(newFeesForm.base_country_id), trigger: 'change'},
  ],
  fees: [
...
const doNewFeesRequest = async ():Promise<(formInstnace: (FormInstance | undefined), loader: any) => void>=>{
  const requestData =  await supabase.from('fees')
      .insert([FeeRequestMappper.newFormToRequest(newFeesForm)])
      .select();
  return (formInstnace : FormInstance | undefined , loader: any)=>{
    if(!requestData.error){
      resetForm(formInstnace)
      ElNotification({
        title: "Nouveau Frais Alsaba",
        message: "Le Nouveau frais  ajouté avec success",
        type: 'success',
      })
      Logger.info("New Fees Added "+requestData.data)
    }else{
      ElNotification({
        title: "Nouveau Frais Alsaba",
      ...
      Logger.error("New Fees Error "+requestData.error)
    }
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async(valid, fields) => {
    if (valid) {
      const loader = Loader()
      const _handleError = await doNewFeesRequest()
      _handleError(formEl , loader)
      console.log(newFeesForm)
      setTimeout(()=>loader?.close() , 600)
    } else {
    ...
    <el-collapse-item name="1" >
      ...
      >
        <el-form-item label="pays de base" prop="base_country_id">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun pays trouvé" placeholder="pays de base" v-model="newFeesForm.base_country_id">
            <el-option
        ...
        <el-form-item label="pays de destination" prop="target_country_id">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun pays trouvé" placeholder="pays de destination" v-model="newFeesForm.target_country_id">
            <el-option
                v-for="item in countryData"
                :key="item.id"
                :label="item.country_name"
        ...
        <el-form-item label="type de frais" prop="fees_type">
          <el-select no-match-text="aucune correspondance trouvé" no-data-text="aucun type de frais trouvé" placeholder="type de frais" v-model="newFeesForm.fees_type">
            <el-option
        ...
        <el-form-item label="borne inferieur" prop="lower_bound">
          <el-input v-model="newFeesForm.lower_bound" type="number" />
        </el-form-item>
        <el-form-item label="borne superieure" prop="upper_bound">
          <el-input v-model="newFeesForm.upper_bound" type="number" />
        </el-form-item>
        <el-form-item label="la valeur du frais" prop="fees">
          <el-input v-model="newFeesForm.fees" type="number" />
        </el-form-item>
        <el-form-item>
